...
describe("checkGoalBehind — contribution-rate mode (no targetDate)", () => {
  ...
  // ── DELIBERATELY BROKEN TEST ── ci-gate-verification ────────────────────────
  // This assertion is intentionally wrong: a brand-new goal (0 months old)
  // should NOT be flagged as behind, but we assert the opposite to force a
  // red ci check. Delete this test once the gate has been confirmed to block.
  it("CI-GATE-VERIFY: a brand-new goal with $0 saved is behind [INTENTIONALLY FAILS]", () => {
    const goal = makeContributionGoal({ currentAmount: 0, monthlyContribution: 500, monthsOld: 0 });
    const result = checkGoalBehind(goal);
    // Wrong assertion — this goal is brand-new so behind must be false.
    // We assert true to prove the test gate rejects the PR.
    expect(result.behind).toBe(true);
  });
});

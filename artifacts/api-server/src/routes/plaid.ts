  ...
  try {
    ...
    const connection = await db.transaction(async (tx) => {
      ...
      for (const account of accountResponse.accounts) {
        await tx.insert(plaidAccounts).values(accountValues(account, item.id, userId))
          .onConflictDoUpdate({
            target: plaidAccounts.plaidAccountId,
            set: accountValues(account, item.id, userId),
          });
      }
      return item;
    });
  ...
  try {
    ...
    for (const item of items) {
      ...
      for (const account of refreshed.accounts) {
        await db.insert(plaidAccounts).values(accountValues(account, item.id, userId))
          .onConflictDoUpdate({
            target: plaidAccounts.plaidAccountId,
            set: accountValues(account, item.id, userId),
          });
